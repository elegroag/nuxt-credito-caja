...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    const result = await authSrv.verifyCode(event, payload);
    return CustomResponse.success(result, result.message || "Verificación exitosa.");
  } catch (e: unknown) {
    ...
    return CustomResponse.error(
      err?.data?.error || err?.message || "Error al verificar el código",
