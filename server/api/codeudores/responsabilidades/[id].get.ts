...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    if (!solicitudId) {
      ...
      return CustomResponse.error("Solicitud inválida", "Solicitud inválida");
    }
    ...
    const data = await service.detalleResponsabilidad(numeroDocumento, solicitudId);

    return CustomResponse.success(data, "Detalle de responsabilidad obtenido.");
  } catch (e: unknown) {
    const err = e as { statusCode?: number; message?: string };
    const status = Number(err?.statusCode || 502);
    setResponseStatus(event, Number.isFinite(status) ? status : 502);
    return CustomResponse.error(
      err?.message || "Error al obtener detalle",
