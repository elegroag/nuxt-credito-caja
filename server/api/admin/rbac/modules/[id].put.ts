...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    const data = await rbacAdminService().updateModule(id, body);
    return CustomResponse.success(data, "Módulo actualizado.");
  } catch (e: unknown) {
    ...
    return CustomResponse.error(err?.message || "Error al actualizar módulo", "Error");
  }
});
