...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    const data = await rbacAdminService().getRole(id);
    if (!data) {
      setResponseStatus(event, 404);
      return CustomResponse.error("Rol no encontrado", "Error");
    }
    ...
    return CustomResponse.error(err?.message || "Error al obtener rol", "Error");
  }
});
