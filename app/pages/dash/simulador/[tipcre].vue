  ...
  <div class="mx-auto max-w-6xl p-4 sm:p-8">
    ...
    <div class="mb-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-foreground">Simulador de crédito</h1>
        ...
        <div v-if="lineaSeleccionada" class="flex items-center gap-2">
          <Badge :variant="lineaSeleccionada.estado === 'A' ? 'default' : 'destructive'">
    ...
    <div v-if="loading" class="flex justify-center items-center min-h-[400px]">
      <div class="flex flex-col items-center gap-3">
        ...
        <p class="text-sm text-muted-foreground">Cargando línea de crédito...</p>
      </div>
    </div>
      ...
      <UPageCard class="max-w-md text-center">
        <div class="flex flex-col items-center gap-3">
          <div class="w-12 h-12 bg-destructive/10 rounded-full flex items-center justify-center">
            <UIcon name="i-lucide-alert-circle" class="w-6 h-6 text-destructive" />
          </div>
          ...
          <div class="flex gap-3 mt-2">
            ...
            <UButton size="sm" variant="outline" @click="navigateToLineas"> Volver </UButton>
          </div>
        </div>
      </UPageCard>
    </div>
      ...
      <UPageCard class="border-amber-200 bg-amber-50">
        <div class="flex items-center gap-3">
          ...
          <p class="text-sm text-amber-800">
            <strong>{{ lineaSeleccionada.detalle }}</strong> se encuentra temporalmente inactiva. No
            es posible solicitar este crédito en este momento.
          </p>
        </div>
      </UPageCard>
    </div>
    ...
    <div
      v-if="
        lineaSeleccionada && lineaSeleccionada.estado === 'A' && convenioVerificado && isElegible
      "
      class="mb-6"
    >
      <UPageCard class="border-green-200 bg-green-50">
        <div class="flex items-start gap-3">
          ...
          <div>
            <p class="font-semibold text-green-800">¡Elegible para Crédito Convenio!</p>
            <p class="text-sm text-green-700">
              Tu empresa tiene convenio con COMFACA. Beneficios: Tasa reducida, plazo extendido y
              más.
            </p>
          </div>
        </div>
    ...
    >
      ...
      <UPageCard class="border-primary/20">
        <template #header>
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-muted-foreground">
                Detalles de la línea seleccionada y categoría aplicada
              </p>
            </div>
            <Badge :variant="lineaSeleccionada.estado === 'A' ? 'default' : 'destructive'">
              {{ lineaSeleccionada.estado === "A" ? "Activo" : "Inactivo" }}
        ...
        <div class="space-y-4">
          ...
          <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <dt class="text-xs font-medium text-muted-foreground uppercase">Nombre</dt>
              <dd class="text-sm font-semibold mt-1">
                {{ lineaSeleccionada.detalle }}
              </dd>
            </div>
            <div>
              <dt class="text-xs font-medium text-muted-foreground uppercase">Código</dt>
              <dd class="text-sm font-semibold mt-1 font-mono">
                {{ lineaSeleccionada.tipcre }}
              </dd>
            </div>
            <div>
              <dt class="text-xs font-medium text-muted-foreground uppercase">Plazo máximo</dt>
              <dd class="text-sm font-semibold mt-1">{{ lineaSeleccionada.numcuo }} meses</dd>
            </div>
            <div>
              <dt class="text-xs font-medium text-muted-foreground uppercase">Monto máximo</dt>
              <dd class="text-sm font-semibold mt-1">
                {{ fmt(lineaSeleccionada.valmax) }}
              </dd>
            </div>
            <div>
              ...
              <dd class="text-sm font-semibold mt-1">
                {{ fmt(lineaSeleccionada.estcre) }}
              </dd>
            </div>
            <div>
              <dt class="text-xs font-medium text-muted-foreground uppercase">Pago seguro</dt>
              <dd class="text-sm font-semibold mt-1">
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-sm font-semibold">Tasas por categoría</h3>
              <span v-if="trabajadorCodcat" class="text-xs text-muted-foreground">
                Tu categoría: <span class="font-mono font-semibold">{{ trabajadorCodcat }}</span>
              </span>
            </div>

            <div class="flex flex-wrap gap-2">
              <Badge
                v-for="cat in lineaSeleccionada.categorias || []"
                :key="String(cat.codcat)"
                :variant="
                  categoriaTrabajadorAplicada &&
                  String(categoriaTrabajadorAplicada.codcat).toLowerCase() ===
                    String(cat.codcat).toLowerCase()
                    ? 'default'
                    : 'secondary'
                "
                :class="
                ...
                "
              >
                {{ cat.codcat }}: {{ Number(cat.facfin).toFixed(4) }}% EA
                <UIcon
                  v-if="
                    categoriaTrabajadorAplicada &&
                    String(categoriaTrabajadorAplicada.codcat).toLowerCase() ===
              ...
              <span
                v-if="!lineaSeleccionada.categorias || lineaSeleccionada.categorias.length === 0"
                class="text-xs text-muted-foreground"
              >
                Esta línea no tiene categorías configuradas
              </span>
            </div>

            <!-- Llamado de la categoría aplicada -->
            <div
              v-if="categoriaTrabajadorAplicada"
              class="mt-3 p-3 rounded-lg bg-primary/5 border border-primary/20 flex items-center gap-3"
            >
              <div
                class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0"
              >
                <UIcon name="i-lucide-user-check" class="w-4 h-4 text-primary" />
              </div>
              <div class="text-sm">
                <p class="font-semibold text-primary">
                  Se te aplica la categoría {{ categoriaTrabajadorAplicada.codcat }}
                </p>
                <p class="text-muted-foreground">
                  Tasa:
                  <span class="font-semibold"
                    >{{ Number(categoriaTrabajadorAplicada.facfin).toFixed(4) }}%</span
                  >
                  efectiva anual
                </p>
              </div>
            ...
            <div
              v-else-if="
                trabajadorCodcat &&
                lineaSeleccionada.categorias &&
                lineaSeleccionada.categorias.length > 0
              "
              class="mt-3 p-3 rounded-lg bg-amber-50 border border-amber-200 flex items-center gap-3"
            >
              <UIcon name="i-lucide-alert-triangle" class="w-5 h-5 text-amber-600 shrink-0" />
              <p class="text-sm text-amber-800">
                Tu categoría <span class="font-mono font-semibold">{{ trabajadorCodcat }}</span> no
                está configurada para esta línea.
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <UPageCard class="border-primary/20">
          <template #header>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-muted-foreground">Ingresa la información para calcular</p>
              </div>
            </div>
          </template>
          ...
          <div class="space-y-5">
            <!-- Monto -->
            <UFormField label="Monto (COP)" :hint="`Máx: ${fmt(lineaSeleccionada.valmax)}`">
              <UInput
              ...
              <template #help>
                <div class="space-y-2">
                  <p class="text-xs" :class="montoInput?.hintClass">
                    {{ montoInput?.hint || "Ingresa un monto entre 200.000 y el máximo" }}
                  </p>
                  <UProgress
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              ...
              <!-- Tasa -->
              <UFormField
                :label="tipoTasa === 'anual' ? 'Tasa efectiva anual (EA %)' : 'Tasa mensual (%)'"
                :hint="
                  categoriaTrabajadorAplicada
                    ? `Tasa aplicada por tu categoría ${categoriaTrabajadorAplicada.codcat}`
                    : ''
                "
              >
                <UInput
                  :model-value="tasaInput"
                  type="number"
                  step="0.0001"
                  min="0"
                  :disabled="!!categoriaTrabajadorAplicada"
                  :placeholder="
                    categoriaTrabajadorAplicada
                      ? Number(categoriaTrabajadorAplicada.facfin).toFixed(4)
                      : ''
                  "
                />
              </UFormField>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <UFormField label="Ingresos mensuales" description="Salario bruto">
                ...
                <template #help>
                  ...
                  <p class="text-xs text-muted-foreground">Neto (92%): {{ fmt(ingresosSan) }}</p>
                </template>
              </UFormField>

              <!-- Seguridad social (8% del salario bruto) -->
              <UFormField label="Seguridad social" description="8% del salario bruto">
                <UInput
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <!-- Descuentos -->
              <UFormField label="Descuentos mensuales" description="Obligaciones actuales">
                <UInput v-model.number="descuentosMensuales" type="number" step="10000" min="0" />
              </UFormField>

              <!-- Max endeudamiento -->
              <UFormField label="Máximo endeudamiento" description="Por ley 50% de capacidad">
                <UInput v-model.number="maxEndeudamientoPct" type="number" min="0" max="100" />
              </UFormField>
            </div>
          </div>
        </UPageCard>
        ...
        <div class="space-y-4">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <UPageCard class="bg-accent/5">
              ...
              <p class="text-xs text-muted-foreground mt-1">Máx: {{ fmt(maxCuotaPermitida) }}</p>
            </UPageCard>
          </div>
          ...
          <UPageCard>
            <div class="flex items-center gap-2 mb-2">
              <UIcon name="i-lucide-receipt" class="w-4 h-4 text-muted-foreground" />
              <span class="text-xs font-medium text-muted-foreground uppercase">Total a pagar</span>
            </div>
            <p class="text-xl font-bold">
              {{ fmt(totalPagar) }}
            </p>
            <p class="text-sm text-muted-foreground">Intereses: {{ fmt(intereses) }}</p>
          </UPageCard>

          <!-- Evaluación -->
          <UPageCard :class="apto ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'">
            <div class="flex items-start gap-3">
              <div
              ...
              <div>
                <p class="font-semibold" :class="apto ? 'text-green-800' : 'text-red-800'">
          ...
          <div class="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <UButton variant="outline" size="lg" icon="i-lucide-rotate-ccw" @click="reset">
              Limpiar
            </UButton>
          </div>
          <p
